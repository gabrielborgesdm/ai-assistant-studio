...
import {
  OllamaDownloadStreamResponse,
  OllamaMessageStreamResponse,
} from "@global/types/ollama";
...
export default class OllamaService {
  ...
  removeAssistantMessageHistoryIfEphemeral(
    ...
    if (assistant.ephemeral) {
      ...
      console.log(
        "Ephemeral message, keeping only the system and last message",
      );
  ...
  addsystemBehaviourToHistory(
    ...
    if (
      ...
      history.messages[0].role === MessageRole.SYSTEM
    ) {
      return;
    }
    ...
    history.messages.unshift(
      AssistantMessageFactory(MessageRole.SYSTEM, assistant.systemBehaviour),
    );
  }
