...
export const useHandleForm = (assistant?: AssistantData): UseHandleForm => {
  ...
  const [selectedModel, setSelectedModel] = useState<OllamaModel | undefined>(
    undefined,
  );
  ...
  const onSubmit = async (values: AssistantFormData): Promise<void> => {
    ...
    setIsLoading(true);
    const savedAssistant = await window.api.assistants.saveAssistant(
      values,
      assistant?.id,
    );
    loadAssistants();
  ...
  const validateTitle = (title: string): boolean => {
    const assistantsNames = assistants.map((assistant) =>
      assistant.title.toLowerCase(),
    );
